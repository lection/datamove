...
public class ${className}Saver extends AbstractSaver{
    ...
    public void save(Object object) throws SQLException{
        ...
        if(obj.get${pro[key][2]}() != null)
            pre.set${pro[key][1]?if_exists}(${key_index+1},new Timestamp(obj.get${pro[key][2]?if_exists}().getTime()));
        else pre.set${pro[key][1]?if_exists}(${key_index+1},null);
        <#elseif (pro[key][1]?exists&&pro[key][1]=="Date")>
        if(obj.get${pro[key][2]?if_exists}() != null)
            pre.set${pro[key][1]}(${key_index+1},new Date(obj.get${pro[key][2]?if_exists}().getTime()));
        else pre.set${pro[key][1]?if_exists}(${key_index+1},null);
        <#elseif (pro[key][2]?ends_with("getId"))>
        if(obj.get${key?cap_first}() != null)
            pre.set${pro[key][1]?if_exists}(${key_index+1},obj.get${pro[key][2]?if_exists}());
        else pre.setNull(${key_index+1},Types.BIGINT);
        <#else>
        pre.set${pro[key][1]?if_exists}(${key_index+1},obj.get${pro[key][2]?if_exists}());
        </#if>
        </#list>
